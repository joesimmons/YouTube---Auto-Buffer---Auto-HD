...
(function (undefined) {
    ...
    function msg(t, h) {
        ...
        if (!box.exists) {
            ...
            //JSL('#' + msg_close + '_close').addEvent('click', msg_close);
        } else {
            box.innerHTML += t;
        }
    ...
    function setPref(str, query, value) {
        str = (str.replace(new RegExp('[?&]?' + query + '=[^&]*'), '') + '&' + query + '=' + value).replace(/^&+|&+$/, '');
        return {
            setPref : function (query, value) {
                return setPref(str, query, value);
            },
            value : function () {
                return str;
            }
    ...
    function main(player) {
        var playerClone = player.cloneNode(true),
            ...
            playIfPlaylist = rList.test(URL) && GM_config.get('autoplayplaylists') === true,
            fv = playerClone.getAttribute('flashvars'),
            isHTML5 = JSL('video.html5-main-video').exists,
            args, arg;

        if (typeof uw.ytplayer !== 'undefined') {
        ...
        if (isHTML5) {
            ...
            // copy the 'ytplayer.config.args' into the flash vars
            if (args) {
                for (arg in args) {
                    setPref( fv, arg, encodeURIComponent(args[arg]) );
                }
            }
            ...
            // edit the flashvars
            fv = setPref(fv, 'enablejsapi', '1')                                                        // enable JS API
                .setPref('vq', userQuality)                                                             // set the quality
                ...
                .setPref('dash', (GM_config.get('disableDash') === true ? '0' : '1') ).value();         // enable/disable dash playback
